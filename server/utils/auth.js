...
module.exports = {
  ...
  authMiddleware: function ({ req }) {
    ...
    try {
      ...
      const { authenticatedPerson } = jwt.verify(token, secret, {
        maxAge: expiration,
      }); // Extract the authenticatedPerson from the token payload (contains user data like ID and username)
  ...
  signToken: function ({ email, username, _id }) {
    // extracts only the username email id from the user object passed to this function from the addUser Resolver
