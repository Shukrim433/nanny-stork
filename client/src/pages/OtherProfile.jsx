...
const OtherProfile = () => {
  ...
  const [addFriend, { loading: mutationLoading, error: mutationError }] =
    useMutation(ADD_FRIEND);
  const [removeFriend, { loading: removeLoading, error: removeError }] =
      ...
      <div className="container mx-auto p-4">
        <div className="bg-white shadow-lg rounded-lg p-6">
          ...
          <h3 className=" font-bold mb-4">
            <button className=" hover:text-gray-700 p-4">
            ...
            <span className="p-4">
              {Auth.loggedIn() ? (
                !isFriend ? (
                  <Button
                    onClick={useAddFriend}
                    className="flex flex-row justify-center ml-3"
                  >
                    Follow{" "}
                    <svg
          ...
          <div>{user.tracker && <OtherPregnancyTracker user={user} />}</div>
