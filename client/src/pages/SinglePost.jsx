...
const SinglePost = () => {
  ...
  const [savePost, { loading: saveLoading, error }] =
    useMutation(ADD_SAVED_POST);
  const [removeSavePost, { loading: removeSaveLoading, error: loadingError }] =
  ...
  if (Auth.loggedIn()) {
    ...
    useEffect(() => {
      ...
      if (user.savedPosts) {
        // checks if post.savedPosts exists
        const isSaved = user.savedPosts.some(
          (savedPost) => savedPost._id === postId
        ); // checks if this post is saved
        setSaved(isSaved);
      }
    }, [user, postId]);
  }
